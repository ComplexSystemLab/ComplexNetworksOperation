...
def generate_points(
        set_densityDistance: float = None,
        set_numPoints: int = None,
        set_circleRadius: float = None,
        circle_origin: np.ndarray = np.array([0, 0]),
        num_samples=30,
        distribution='Normal',
        algorithm: Optional[str] = None,
        sigma: float = 1,
        hierarchical_network_type=False,
    ...
    if set_densityDistance is not None and set_numPoints is not None and set_circleRadius is None:  # 如果指定了密度距离、总数，估算范围半径
        ...
        if dim == 2:
            ...
            num_points = round(a / set_densityDistance) if round(a / set_densityDistance) > 0 else 1
        if dim == 3:
            a = set_circleRadius * (4 / 3 * np.pi) ** (1 / 3)
            num_points = round(a / set_densityDistance) if round(a / set_densityDistance) > 0 else 1
        if dim == 1:
            num_points = round(set_circleRadius / set_densityDistance) if round(set_circleRadius / set_densityDistance) > 0 else 1
            pass  # if
        density_distance = set_densityDistance
            ...
            match algorithm:
                ...
                case None:
                    if set_numPoints is not None:
                        algorithm = 'K-means'
                        return _generate_points_used_PoissonDisk_random_distribution_by_Kmeans_algorithm(circle_origin, circle_radius, num_points)
                    if set_densityDistance is not None and set_numPoints is None and set_circleRadius is not None:
                        algorithm = 'Bridson'
                        return _generate_points_used_PoissonDisk_random_distribution_by_Bridson_algorithm(circle_origin, circle_radius, density_distance, num_samples)
                        pass  # if
...
def _generate_points_used_PoissonDisk_random_distribution_by_Bridson_algorithm(origin, radius, min_distance, num_samples=15):
    ...
    while np.linalg.norm(first_sample) > radius:
        first_sample = np.array([np.random.uniform(-radius, radius), np.random.uniform(-radius, radius)])
    ...
    while active_list:
        ...
        for _ in range(num_samples):
            ...
            if np.linalg.norm(new_sample) <= radius and _is_valid_sample_by_Bridson_algorithm(new_sample, radius, min_distance, samples[:sample_count], grid, cell_size):
    ...
    # 偏移原点
    samples = samples[:sample_count] + origin
    return samples
    pass  # function
...
@njit
def _is_valid_sample_by_Bridson_algorithm(sample, radius, min_distance, samples, grid, cell_size):
    """
    ...
    for neighbor_index in _get_neighbor_indices(grid_index, grid.shape):
        if grid_index[0] >= 0 and grid_index[0] < grid.shape[0] and grid_index[1] >= 0 and grid_index[1] < grid.shape[1] and grid[neighbor_index] != -1:
...
@njit
def _generate_hierarchical_network_nodes(circle_origin: np.ndarray, circle_radius: float, num_points: int, num_external_nodes: int):
    """
    ...
    """
    points = np.empty((num_points, 2))
    external_points = np.empty((num_external_nodes, 2))
    internal_points = np.empty((num_points - num_external_nodes, 2))

    # 生成外部节点
    for i in range(num_external_nodes):
        theta = np.random.uniform(0, 2 * np.pi)
        r = circle_radius
        external_points[i] = [r * np.cos(theta) + circle_origin[0], r * np.sin(theta) + circle_origin[1]]

    # 生成内部节点
    for i in range(num_points - num_external_nodes):
        theta = np.random.uniform(0, 2 * np.pi)
        r = np.sqrt(np.random.uniform(0, 1)) * circle_radius
        internal_points[i] = [r * np.cos(theta) + circle_origin[0], r * np.sin(theta) + circle_origin[1]]

    points[:num_external_nodes] = external_points
    points[num_external_nodes:] = internal_points
